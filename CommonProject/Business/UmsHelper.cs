...
namespace CommonProject.Business
{
    public class UmesHelper
    {
        ...
        public static void FindUmesInfo(string boardNo)
        {
            ...
            if (!_Is_Wip)
            {
                ...
                if (_RuleItem == null)
                {
                    ...
                    _RuleItem = SingletonHelper.PvsInstance.GetBarodeRuleItemsByRuleNo(UsapHelper.ProductID.PadLeft(tmp));
                }
            }
            else
            {
        ...
        public static bool CompareModel()
        {
            ...
            var arr = UsapHelper.ProductID.Split('-');
            if (arr.Length >= 3 && arr[2].Contains("000SS"))
            {
                usapModel = string.Format("{0}-{1}-{2}", arr[0], arr[1], arr[2].Substring(3, arr[2].Length - 3));
            ...
            return umesModel.PadLeft(min).Equals(usapModel.PadLeft(min), StringComparison.OrdinalIgnoreCase);
        }
